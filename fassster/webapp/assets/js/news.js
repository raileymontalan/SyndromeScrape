...
function updateNewsMap() {
    if (newsmap = 0) dataLayer.getLayers()[0].setStyle(Newsstyle);
    else showNewsMap();
}
...
function showNewsMap() {
    ...
    var data_url = "assets/tempdata/2017-01.json";
...
function requestNewsData(s,l,f){
    $('.modal-loading').show();
    //remove overlayed layers and popups
    dataLayer.clearLayers();
    mymap.closePopup();
    dataType = "News";
    var data_url = "assets/tempdata/final_news.json";
    $.getJSON(data_url, function(data) {
        console.log(data);
        renderNews(data);
    }).fail(
    ...
    function renderNews(data) {
        ...
        mapslider.update({
            ...
            onChange: function (data) {
                //viewNewsToggle = $('#viewNews').is(':checked'); 
                if (viewNewsToggle) updateNewsMap();
                else{
...
function Newsstyle(feature) {
    ...
    return {
        fillColor: NewsColor(count, 0, provMaxCount),
        weight: 0.7,
        opacity: 0.7,
        color: 'gray',
        fillOpacity: 0.75
...
$('#viewNewsShow').click(function() {
    ...
    //if (!this.checked) requestNewsData();
});
$('#viewNewsHide').click(function() {
    $( "#viewNewsShow" ).show();
    $( "#viewNewsHide" ).hide();
    newsmap = 0;
    //pause();
